...
def resetButton(win, man, woman):
    #msg = "RESET"
    #button = pygame.draw.rect(win, (255, 255, 255), (1226, 15, 100, 45))
    if event.type == pygame.MOUSEBUTTONDOWN:
        x, y = event.pos
...
# plat_s = platform(100, win_y-200, 250, win_y-200,(0,150,255))
plat_s = platforms()
...
# plat_s.add(platform(200,0
man = player(startPos[0], startPos[1], 64, 64)
woman = player_2(0, 0, 64, 64)
m_bullets = []
w_bullets = []
...
while run:
    clock.tick(80)
    womanPos = read_pos(n.send(make_pos((man.x, man.y)))) #!!!!! here maybe mistake instead p.x, p.y give there man.x,man.y
    woman.x = womanPos[0]
    woman.y = womanPos[1]
    ...
    for m_bullet in m_bullets:
        if m_bullet.y - m_bullet.height < woman.hitbox[1] and m_bullet.y + m_bullet.height > woman.hitbox[1]:
            if m_bullet.x + m_bullet.width > woman.hitbox[0] and m_bullet.x - m_bullet.width < woman.hitbox[0] - 87:
                woman.hit()
    ...
    for w_bullet in w_bullets:
        if w_bullet.y - w_bullet.height < man.hitbox[1] and w_bullet.y + w_bullet.height > man.hitbox[1]:
            if w_bullet.x + w_bullet.width > man.hitbox[0] and w_bullet.x - w_bullet.width < man.hitbox[0] - 87:
                man.hit()
